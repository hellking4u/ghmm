...
ghmm_alphabet::ghmm_alphabet():
  XMLIO_ElementArrayObject<ghmm_symbol>()
{
  init_members();
}

ghmm_alphabet::ghmm_alphabet(const string& tag, XMLIO_Attributes &attributes):
  XMLIO_ElementArrayObject<ghmm_symbol>(tag,attributes)
{
  init_members();
}
...
{
  cout<<toString()<<":"<<endl;
  for (XMLIO_ElementArrayObject<ghmm_symbol>::const_iterator pos=begin();pos!=end();++pos)
    if (*pos!=NULL)
      (*pos)->print();
