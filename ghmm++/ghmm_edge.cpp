...
XMLIO_Object* ghmm_edge::XMLIO_startTag(const string& tag, XMLIO_Attributes &attributes)
{
...
void ghmm_edge::XMLIO_endTag(const string& tag)
{
  ...
  if (tag=="attr")
      ...
      if (last_attr->name=="transitions" && !last_attr->empty())
	{
	  ...
	  XMLIO_ElementArrayObject<XMLIO_StringObject>* int_array;
	  int_array=dynamic_cast<XMLIO_ElementArrayObject<XMLIO_StringObject>*>(last_attr->front());
	  /* is it possible to get transition probs ? */
	  if (int_array!=NULL)
	    {
	      for(XMLIO_ElementArrayObject<XMLIO_StringObject>::const_iterator pos=int_array->begin();
		  pos!=int_array->end();
		  ++pos)
		{
