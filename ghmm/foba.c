    ...
    for (t = 1; t < len; t++) {
      ...
      for (i = 0; i < mo->N; i++) {
        if (!(mo->model_type & kSilentStates) || !(mo->silent[i])) {
          ...
          if (e_index != -1) {
            alpha[t][i] =
              ghmm_d_forward_step (&mo->s[i], alpha[t - 1], mo->s[i].b[e_index]);
            scale[t] += alpha[t][i];
          }
          else {
            alpha[t][i] = 0;
      ...
      if (mo->model_type & kSilentStates) {
        for (i = 0; i < mo->topo_order_length; i++) {
          ...
          /*printf("  akt_ state %d\n",id);*/
          /*printf("\nin stepforward\n");*/
          alpha[t][id] = ghmm_d_forward_step (&mo->s[id], alpha[t], 1);
          /*printf("\nnach stepforward\n");*/
          scale[t] += alpha[t][id];
        }
      }
...
int ghmm_d_forward_descale (double **alpha, double *scale, int t, int n,
                  double **newalpha)
{
# define CUR_PROC "ghmm_d_forward_descale"
  int i, j, k;
  /*printf("\nAngekommen, t=%i, n=%i\n",t,n);*/
  for (i = 0; i < t; i++) {
    /*printf("i=%i\n",i);*/
...
# define CUR_PROC "ghmm_d_logp"
  ...
  /* run ghmm_d_forward */
  if (ghmm_d_forward (mo, O, len, alpha, scale, log_p) == -1) {
    mes_proc ();
    goto STOP;
  }
    ...
    *log_p = -log (1 / scale[0]);
    ...
    for (t = 1; t < len; t++) {
      ...
      if (mo->model_type & kSilentStates) {
        for (i = 0; i < mo->topo_order_length; i++) {
          id = mo->topo_order[i];
          alpha_curr_col[id] = ghmm_d_forward_step (&mo->s[id], alpha_curr_col, 1);
      ...
      for (i = 0; i < mo->N; i++) {
        if (!(mo->model_type & kSilentStates) || !(mo->silent[i])) {
          ...
          if (mo->s[i].label == label[t]) {
            ...
            if (-1 != e_index) {
              alpha[t][i] =
                ghmm_d_forward_step (&mo->s[i], alpha[t - 1],
                                  mo->s[i].b[e_index]);
...
# define CUR_PROC "ghmm_dl_logp"
  ...
  ARRAY_CALLOC (scale, len);
  /* run ghmm_d_forward */
  if (ghmm_dl_forward (mo, O, label, len, alpha, scale, log_p) == -1) {
    mes_proc ();
    goto STOP;
  }
  ...
  *log_p = -log (1 / scale[0]);
  ...
  for (t = 1; t < len; t++) {
    ...
    if (mo->model_type & kSilentStates) {
      for (i = 0; i < mo->topo_order_length; i++) {
        id = mo->topo_order[i];
        alpha_curr_col[id] = ghmm_d_forward_step (&mo->s[id], alpha_last_col, 1);
        scale[t] += alpha_curr_col[id];
      }
    }
