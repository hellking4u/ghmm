/*******************************************************************************
  ...
  created      : TIME: 02:02:35     DATE: Sun 20. June 2004
  $Id$
...
#define CUR_PROC "discrime_galloc"
  ...
  if (!m_calloc(*matrix_b, noC)) {mes_proc(); goto STOP;}
  for (k=0; k<noC; k++) {
    if (!m_calloc((*matrix_b)[k], sqs[k]->seq_number)) {mes_proc(); goto STOP;}
    for (l=0; l < sqs[k]->seq_number; l++) {
      if (!m_calloc((*matrix_b)[k][l], noC)) {mes_proc(); goto STOP;}
      for (m=0; m<noC; m++) {
	if (!m_calloc((*matrix_b)[k][l][m], mo[m]->N)) {mes_proc(); goto STOP;}
	for (i=0; i<mo[m]->N; i++)
	  if (!m_calloc((*matrix_b)[k][l][m][i], (int)pow(mo[m]->M, mo[m]->s[i].order+1))) {
	    mes_proc(); goto STOP;
	  }
      }
    }
  ...
  if (!m_calloc(*matrix_a, noC)) {mes_proc(); goto STOP;}
  for (k=0; k<noC; k++) {
    if (!m_calloc((*matrix_a)[k], sqs[k]->seq_number)) {mes_proc(); goto STOP;}
    for (l=0; l < sqs[k]->seq_number; l++) {
      if (!m_calloc((*matrix_a)[k][l], noC)) {mes_proc(); goto STOP;}
      for (m=0; m<noC; m++)
	if (!m_calloc((*matrix_a)[k][l][m], mo[m]->N*mo[m]->N)) {mes_proc(); goto STOP;}
    }
  }

  /* allocate memory for matrix_pi */
  if (!m_calloc(*matrix_pi, noC)) {mes_proc(); goto STOP;}
  for (k=0; k<noC; k++) {
    if (!m_calloc((*matrix_pi)[k], sqs[k]->seq_number)) {mes_proc(); goto STOP;}
    for (l=0; l < sqs[k]->seq_number; l++) {
      if (!m_calloc((*matrix_pi)[k][l], noC)) {mes_proc(); goto STOP;}
      for (m=0; m<noC; m++)
	if (!m_calloc((*matrix_pi)[k][l][m], mo[m]->N)) {mes_proc(); goto STOP;} 
    }
  }
  ...
  if (!m_calloc(*log_p, noC)) {mes_proc(); goto STOP;}
  for (k=0; k<noC; k++) {
    if (!m_calloc((*log_p)[k], sqs[k]->seq_number)) {mes_proc(); goto STOP;}
    for (l=0; l < sqs[k]->seq_number; l++)
      if (!m_calloc((*log_p)[k][l], noC)) {mes_proc(); goto STOP;}
  }

  /* allocate memory for outer derivatives */
  if (!m_calloc(*omega, noC)) {mes_proc(); goto STOP;}
  for (k=0; k<noC; k++)
    if (!m_calloc((*omega)[k], sqs[k]->seq_number)) {mes_proc(); goto STOP;}
  
  /* allocate memory for omega tilde. NB: size(omega)*noC == size(omegati)*/
  if (!m_calloc(*omegati, noC)) {mes_proc(); goto STOP;}
  for (k=0; k<noC; k++) {
    if (!m_calloc((*omegati)[k], sqs[k]->seq_number)) {mes_proc(); goto STOP;}
    for (l=0; l < sqs[k]->seq_number; l++)
      if (!m_calloc((*omegati)[k][l], noC)) {mes_proc(); goto STOP;}
  }
  ...
  for (k=0; k<noC; k++) {
    ...
    for (l=0; l<seq_no; l++) {
	 ...
	 first determine the maximum */
      for (m=0; m<noC; m++) {
	if (m!=k && (1.0==max || max < (log_p[k][l][m]+log(mo[m]->prior)))) {
...
/*       printf("omega[%d][%d] = %Lg\n",k ,l, omega[k][l]); */
	...
	if (m==k)
	  ...
	  omegati[k][l][m] = omega[k][l] * expl(log_p[k][l][m] - sum) * (long double)mo[m]->prior;
...
#define CUR_PROC "discrime_precompute"
  ...
  for (k=0; k < noC; k++) {
	...
	gradescent_compute_expectations(mo[m], alpha, beta, scale, sq->seq[l],
					seq_len, expect_b[k][l][m],
					expect_a[k][l][m], expect_pi[k][l][m]);
...
#define CUR_PROC "discrime_compute_performance"
  ...
  if (!m_calloc(logp, noC)) {printf("error\n"); mes_proc(); goto STOP;}
...
#define CUR_PROC "discrime_update_pi_gradient"
  ...
  if (!m_calloc(pi_old, mo[class]->N)) {mes_proc(); goto STOP;}
  if (!m_calloc(pi_new, mo[class]->N)) {mes_proc(); goto STOP;}
...
#define CUR_PROC "discrime_update_a_gradient"
  ...
  if (!m_calloc(a_old, mo[class]->N)) {mes_proc(); goto STOP;}
  if (!m_calloc(a_new, mo[class]->N)) {mes_proc(); goto STOP;}
  ...
  for (i=0; i<mo[class]->N; i++) {
	...
	if (m==class)
	  for (l=0; l<sq->seq_number; l++)
	    sum += omega[class][l] * expect_a[class][l][+class][i*mo[class]->N+j_id];
	else
	  for (l=0; l<sq->seq_number; l++)
	    sum -= omegati[m][l][class] * expect_a[m][l][class][i*mo[class]->N+j_id];
      }
      /* check for valid new parameter */
...
#define CUR_PROC "discrime_update_b_gradient"
  ...
  if (!m_calloc(b_old, mo[class]->M)) {mes_proc(); goto STOP;}
  if (!m_calloc(b_new, mo[class]->M)) {mes_proc(); goto STOP;}
...
/*----------------------------------------------------------------------------*/
void discrime_update_pi_closed(model** mo, sequence_t** sqs, int noC, int class,
...
#define CUR_PROC "discrime_update_pi_closed"
  ...
  for (i=0; i<mo[class]->N; i++) {
	...
	for (l=0; l<sq->seq_number; l++)
	  lagrangian += lfactor * omegati[m][l][class] * expect_pi[m][l][class][i];
    }
  }
...
/*----------------------------------------------------------------------------*/
void discrime_update_a_closed(model** mo, sequence_t** sqs, int noC, int class,
			      double lfactor, double**** expect_a,
			      long double** omega, long double*** omegati) {
#define CUR_PROC "discrime_update_a_closed"
  ...
  for (i=0; i<mo[class]->N; i++) {
	...
	if (m==class)
	  for (l=0; l<sq->seq_number; l++)
	    lagrangian -= omega[class][l] * expect_a[class][l][class][i*mo[class]->N+j_id];
	else
	  for (l=0; l<sq->seq_number; l++)
	    lagrangian += lfactor * omegati[m][l][class] * expect_a[m][l][class][i*mo[class]->N+j_id];
      }
    }
	...
	if (m==class)
	  ...
	  for (l=0; l<sq->seq_number; l++)
	    sum += lfactor * omegati[m][l][class] * expect_a[m][l][class][i*mo[class]->N+j_id];
      }
      /* check for valid new parameter */
      a_old[j] = mo[class]->s[i].out_a[j];
      if (fabs(lagrangian) == 0.0)
...
/*----------------------------------------------------------------------------*/
void discrime_update_b_closed(model** mo, sequence_t** sqs, int noC, int class,
			      double lfactor, double***** expect_b,
			      long double** omega, long double*** omegati) {
  ...
  for (i=0; i<mo[class]->N; i++) {
	...
	for (m=0; m<noC; m++) {
	  if (m==class)
	    ...
	    for (l=0; l<sqs[m]->seq_number; l++)
	      lagrangian += lfactor * omegati[m][l][class] * expect_b[m][l][class][i][h];
	}
      }
	...
	for (m=0; m<noC; m++) {
	  if (m==class)
	    ...
	    for (l=0; l<sqs[m]->seq_number; l++)
	      sum += lfactor * omegati[m][l][class] * expect_b[m][l][class][i][h];
	}
	/* check for valid new parameters */
	b_old[h] = mo[class]->s[i].b[h];
	if (fabs(lagrangian) == 0.0)
...
/*----------------------------------------------------------------------------*/
void discrime_find_factor(model* mo, sequence_t** sqs, int noC, int k, double lfactor,
			  double**** expect_pi, double**** expect_a, double***** expect_b,
			  long double** omega, long double*** omegati) {
#define CUR_PROC "driscrime_find_factor"
  ...
  for (i=0; i<mo->N; i++) {
	...
	if (m==k)
	  ...
	  for (l=0; l<sq->seq_number; l++)
	    other += lfactor * omegati[m][l][k] * expect_a[m][l][k][i*mo->N+j_id];
      }
      if (self < other)
	lfactor *= self/other;
      //printf(" A[%d]: %g - %g = %g \t %g\n", k, self, other, self-other, lfactor);
...
/*----------------------------------------------------------------------------*/
int discrime_onestep(model** mo, sequence_t** sqs, int noC, int grad, int class) {
#define CUR_PROC "driscrime_onestep"
  ...
  if (-1==discrime_precompute(mo, sqs, noC, expect_b, expect_a, expect_pi, log_p)) {
  ...
  if (grad) {
    ...
    discrime_update_pi_gradient(mo, sqs, noC, class, expect_pi, omega, omegati);
    discrime_update_a_gradient(mo, sqs, noC, class, expect_a, omega, omegati);
    discrime_update_b_gradient(mo, sqs, noC, class, expect_b, omega, omegati);
  }
  else {
    ...
    discrime_find_factor(mo[class], sqs, noC, class, lfactor, expect_pi, expect_a, expect_b, omega, omegati);
    lfactor *= .99;
    printf("Calculated L: %g\n", lfactor);
    
    /* updating parameters */
    discrime_update_pi_closed(mo, sqs, noC, class, lfactor, expect_pi, omega, omegati);
    discrime_update_a_closed(mo, sqs, noC, class, lfactor, expect_a, omega, omegati);
    discrime_update_b_closed(mo, sqs, noC, class, lfactor, expect_b, omega, omegati);
  }
...
#define CUR_PROC "driscriminative"
  ...
  if (!m_calloc(falseP, noC)) {mes_proc(); return -1;}
  if (!m_calloc(falseN, noC)) {mes_proc(); m_free(falseP); return -1;}
  ...
  for (k=0; k<noC; k++) {
    firstrun = 1;
    step = 0;
    if (gradient) lambda=.3;
  
    while ((last_perf < cur_perf || cur_cer < last_cer) && step++<75 || firstrun) {
      if (firstrun) firstrun=0;
      else model_free(&last);
