/*******************************************************************************
  author       : Frank Nübel
  filename     : /homes/hmm/lib/scanner.c
  created      : TIME: 11:40:33     DATE: Tue 27. August 1996
  last-modified: TIME: 15:43:23     DATE: Wed 30. August 2000
*******************************************************************************/
...
static int scanner_type( char* type, int* size ) {
  if( !strcmp( type, "char" ) ) 
    { *size = sizeof(char);   return(SCANNER_TYPE_CHAR); }
  if( !strcmp( type, "int" ) ) 
    { *size = sizeof(int);    return(SCANNER_TYPE_INT); }
  if( !strcmp( type, "double" ) ) 
    { *size = sizeof(double); return(SCANNER_TYPE_DOUBLE); }
  if( !strcmp( type, "edouble" ) ) 
    { *size = sizeof(double); return(SCANNER_TYPE_EDOUBLE); }
  if( !strcmp( type, "string" ) ) 
    { *size = sizeof(char*);   return(SCANNER_TYPE_STRING); }
  if( !strcmp( type, "cstring" ) ) 
    { *size = sizeof(char*);   return(SCANNER_TYPE_CSTRING); }
  *size = 0;
  return(0);  
} /* scanner_type */
...
static int scanner_digit( int* val, scanner_t* s, int radix, int expect ) {
  switch( radix ) {
  case 10 : 
    if( '0' <= s->c && s->c <= '9' ) *val = *val*10 + s->c - '0';
    else if( expect ) { scanner_error( s, "decimal digit expected" ); return(-1); }
    else return(1); 
    break;
  case 16 : 
    if( s->c >= '0' && s->c <= '9' ) *val = *val*16 + s->c - '0';
    else if( 'A' <= s->c  && s->c <= 'F' ) *val = *val*16 + s->c - 'A' + 10;
    else if( 'a' <= s->c  && s->c <= 'f' ) *val = *val*16 + s->c - 'a' + 10;
    else if( expect ) { scanner_error( s, "decimal digit expected" ); return(-1); }
    else return(1); 
    break;
  case 2 : 
    if( s->c >= '0' && s->c <= '1' ) *val = *val*2 + s->c - '0';
    else if( expect ) { scanner_error( s, "binary digit expected" ); return(-1); }
    else return(1); 
    break;
  case 8 : 
    if( s->c >= '0' && s->c <= '7' ) *val = *val*8 + s->c - '0';
    else if( expect ) { scanner_error( s, "octal digit expected" ); return(-1); }
    else return(1); 
    break;
  default: return(1); 
  }
...
static int scanner_nextchar( scanner_t* s, int expect ) {
  if( !s || s->err || s->eof ) return(0);
  while( s->pos + 1 >= s->txtlen ){
    int mes_stat = mes_ability( 0 );
    int err = m_realloc( s->txt, s->txtlen+256 );
    mes_ability( mes_stat );
    
    if( err ) { scanner_error( s, "line too long" ); return(-1); }
    else s->txtlen += 256;
  }
  s->txt[s->pos] = s->c;
  if( s->c == '\n' ) {
  ...
  scanner_fgetc( s );
  if( s->eof && expect )
    { scanner_error( s, "unexpected end of file" ); return(-1); }
  return(0);
} /* scanner_nextchar */

/*----------------------------------------------------------------------------*/
static int scanner_nextcchar( scanner_t* s ){
  if( !s || s->eof || s->err ) return(0);
  if( s ) s->esc = 0;
  if( scanner_nextchar( s, 1 ) ) return(-1);
  if( s->c - '\\' ) return(0);
  else {
    int radix  = 0;
    int digits = 0;
    ...
    if( '0' <= s->c && s->c <= '7' ) {
      ...
      case '"'  : s->c = '\"'; break;
      case '?'  : s->c = '\?'; break;
      case 'd'  : radix = 10; digits = 3; break;
      case 'x'  : radix = 16; digits = 2; break; 
      case '_'  : radix = 2;  digits = 8; break; 
      default: return(0);
      } 
      if( !radix ) return(0);
    }
    while( digits-- ) { 
      if( scanner_nextchar( s, 1 ) ) return(-1);
      if( scanner_digit( &val, s, radix, 1 ) ) return(-1);
    }
    s->c = val;
  }
  return(0);
...
static int scanner_skipspace(scanner_t* s) {
  if( s->eof || s->err ) return(0);
  while( !s->eof ) {
    if( s->c == '#' ) { /* skip comment */
      do {
	if( scanner_nextchar( s, 0 ) ) return(-1);
      } while( !s->eof && s->c - '\n' );
    }
	...
	if( scanner_nextchar( s, 0 ) ) return(-1);
      ...
      if( !s->eof && scanner_nextchar( s, 0 ) ) return(-1);
      if( !s->eof && scanner_nextchar( s, 0 ) ) return(-1);
    } /* FN: 22.10.97 */
...
static double scanner_get_length(scanner_t* s, double resolution) {
  ...
  if( s->c - ';' ) {
    if( resolution <= 0.0 ) {scanner_error( s, "resolution not set" ); return(0);}
    s->resolution_used = 1;
    if( scanner_get_id(s) ) return(0);
    if( !strcmp(s->id,"INCH") ) val *= resolution;
    else if( !strcmp(s->id,"CM") ) val*=(resolution/2.54);
    else if( !strcmp(s->id,"MM") ) val*=(resolution/25.4);
    else { scanner_error(s,"unknown length unit"); return(0); }
  }
  return val;
} /* scanner_get_length */
...
#define CUR_PROC "scanner_alloc"
  ...
  if( !m_calloc( s, 1 ) ) {mes_proc();goto STOP;}
  
  s->txtlen = 256;
  s->idlen  = 256;
  if( !(s->fp = mes_fopen( filename, "rt" )) ) {mes_proc();goto STOP;}
  if( !(s->txt = mes_malloc(s->txtlen)) ) {mes_proc(); goto STOP;}
  if( !(s->id = mes_malloc(s->txtlen)) ) {mes_proc(); goto STOP;}
  ...
  scanner_fgetc( s );
  if( scanner_skipspace( s ) ) goto STOP;
  return( s );
STOP:
  scanner_free( &s );
  return(NULL);  
...
int scanner_consume( scanner_t* s, char ch ){
  ...
  else if( scanner_nextchar( s, 0 ) ) return(-1);
  if( scanner_skipspace(s) ) return(-1);
  return(0);
} /* scanner_consume */
...
int scanner_get_name( scanner_t* s ){
  ...
  while( m_scanner_isxalpha(s->c) || m_scanner_isdigit(s->c) ) {
    while( pos+1 >= s->idlen ){
      ...
      int err = m_realloc( s->txt, s->txtlen+256 );
      mes_ability( mes_stat );
      if( err ) { scanner_error( s, "identifier too long" ); return(-1); }
      else s->idlen += 256;
    }
    s->id[pos++] = s->c;
    if( scanner_nextchar( s, 0 ) ) return(-1);
...
# define CUR_PROC "scanner_get_string"
  ...
  if( s->eof || s->c - '"' ) { scanner_error( s, "string expected" ); goto STOP; }
  if( !m_malloc( val, maxlen ) ){ 
    scanner_error( s, "Not enough memory to read string" ); goto STOP;
  }
  
  while( !s->eof && s->c == '"' ) {
    if( cmode ) { if( scanner_nextcchar( s ) ) goto STOP; }
    else { if( scanner_nextchar( s, 1 ) ) goto STOP; }
    while ( s->c - '"' || s->esc ) {
      if( s->eof || ((!s->c || s->c == '\n') && !s->esc ) )
	{ scanner_error( s, "String not closed" ); goto STOP;  }
      if( i+1 == maxlen ) {
	...
	maxlen += 128;
      ...
      if( s->c || len ) val[i++] = s->c;
      if( cmode ) { if( scanner_nextcchar( s ) ) goto STOP; }
      else { if( scanner_nextchar( s, 1 ) ) goto STOP; }
    }
    
    if( scanner_consume( s, '"' ) ) goto STOP;
    if( scanner_skipspace( s ) ) goto STOP;
  }
  val[i++]=0;
  if( m_realloc( val, i ) ) { mes_proc(); goto STOP; }
  if( len ) *len = i;
  return( val );
