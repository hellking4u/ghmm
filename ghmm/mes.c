...
static mes_t* mes_process_get(void) {
  ...
  for(i=mes_process_n-1;i>=0;i--)
    if( mes_process[i] )
      if( mes_process[i]->thread_id==tid ) return(mes_process[i]);
  return(NULL);
} /* mes_process_get */
...
static void mes_process_alloc() {
    mes_t *mpi;
    int tid = getthreadid();
    if( mes_process_get() ) return;
    if( mes_process_n>=MES_PROCESS_MAX ) return;
    if( !mes_process ) return;
    else {
        int i,j = -1;
        for(i=mes_process_n-1;i>=0;i--) 
            if( !mes_process[i] ) {
...
static void mes_arg_free( void ) { 
  ...
  if( mpc->argv ) {
    while( mpc->argc > 0) { 
      mpc->argc--; 
      if( mpc->argv[mpc->argc] ) free( mpc->argv[mpc->argc] ); 
    }
    free( mpc->argv );
    mpc->argv = NULL;
  }
...
static void mes_init_std_path( char* logfile ) { 
  ...
  if( mpc ) {
    if( mpc->std_path ) free( mpc->std_path );
    for( i = logfile?strlen(logfile):0; i && logfile[i-1] - PathSepChar; i-- );
...
void mes_smart( int flags, char* txt, int bytes ) {
  ...
  if( !mes_process_enabled() ) return; 
  if( flags & MES_FLAG_TIME ) mes_time();
  if( !txt ) return;
  if( bytes < 0 ) {
    slen = strlen( txt );
    bytes = slen;
  ...
  if( flags & ( MES_FLAG_FILE | MES_FLAG_TIME) ) {
    ...
    char* logfile = NULL;
    mes_t *mpc = mes_process_get();
    if( mpc ) logfile = mpc->logfile;
    
    if( !logfile ) logfile = mes_default_logfile;
	...
	char bakfile[300];
	if( mpc && mpc->std_path ) sprintf( bakfile, "%s%s", mpc->std_path,
					    mes_default_bak_logfile );
	else sprintf( bakfile, "%s", mes_default_bak_logfile );
	remove( bakfile );
  ...
  for(i=mes_process_n-1;i>=0;i--) {
      ...
      if( mpi )
          if( mpi->thread_id==tid ) {
            if( mpi->logfile ) free( mpi->logfile );
            if( mpi->std_path ) free( mpi->std_path );
            mes_arg_free();
            free( mpi );
            mes_process[i] = NULL;
            return;
...
void mes_init_args( int argc, char* argv[] ) { 
  ...
  if( mpc->argv ) while( mpc->argc < argc && *argv  ) {
    int len = *argv ? strlen( *argv ) + 1 : 1;
    mpc->argv[mpc->argc] = malloc( len );
    if( !mpc->argv[mpc->argc] ) break;
    if( *argv ) memcpy( mpc->argv[mpc->argc], *argv, len );
    else mpc->argv[mpc->argc][0] = 0;
    argv++;
    mpc->argc++;
  } 
} /* mes_init_args */
...
void mes_init_logfile( char* logfile ) {
  ...
  if( tst ) {
    ...
    if( mpc ) {
      ...
      if( mpc->logfile ) free( mpc->logfile );
...
void mes_time( void ) {
  ...
  mes_file( "\n***** " );
  if( mpc ) {
    if( mpc->argc == 1 ) { mes_file( mpc->argv[0] ); mes_file( ":" ); }
    else if( mpc->argc > 1 ) { 
        int i;
        mes_file( mpc->argv[0] );
        mes_file( "(" );
...
void mes_va( int flags, int line, char* xproc, char* proc, char* format, va_list args ) {
  ...
  if( !format && !xproc && !proc ) return;
  if( line + 1 ) mprintf( digstr, sizeof(digstr), "(%u)", line );
  ...
  if( !proc )  proc  = xproc;
  if( proc ) {
    if( flags & MES_FLAG_FILE ) mes_file( xproc );
    if( flags & MES_FLAG_WIN )  mes_win( proc );
    mes_smart( flags, digstr, -1 );
    if( format ) mes_smart( flags, ": ", -1 );
  }
  
  if( !format ) mes_smart( flags, "\n", -1 ); 
  else          mes_aux_va( flags, format, args );
} /* mes_va */
...
void mes_printf( int prot_flags, char* format, ... ) {
  ...
  if( prot_flags & MES_FLAG_TIME ) mes_time();
...
void mes_err( char* txt, int error_nr, char* proc_info ) { 
  ...
  if( error_nr >=0 && error_nr < sizeof(mes_err_txt) / sizeof(*mes_err_txt) ) {
...
void mes_proc_start( char* proc_info ) { 
  ...
  for( i = 0; i < mpc->argc; i++ ) {
    if( !i ) mes_file( "program call name is : " );
    else     mes_aux( MES_FLAG_FILE, "parameter %10d : ", i );
    mes_file( mpc->argv[i] );
    mes_file( "\n" );
  } 
...
int mes_insert( FILE* fp, char src, int cnt ) {
  ...
  if( fp && fp - stdout ) for( i = 0; i < cnt && !mes_fputc( fp, src ); i++ );
  else for( i = 0; i < cnt; i++ ) mes_smart( MES_FLAG_WIN,&src,1);
  if( i == cnt ) return(0);
  return(-1);  
} /* mes_insert */
...
void* mes_malloc( int bytes ) { 
  ...
  res = malloc( bytes );
  if( res ) return( res ); 
  else mes_aux(MES_FLAG_TIME_WIN, "malloc: could not allocate %d bytes\n", bytes);
  return( NULL );  
} /* mes_malloc */
...
void* mes_calloc( int bytes ) { 
  ...
  res = calloc( 1, bytes );
  if( res ) return( res ); 
  else mes_aux( MES_FLAG_TIME_WIN, "calloc: could not allocate %d bytes\n", bytes );
  return( NULL );  
} /* mes_calloc */
...
int mes_realloc( void** mem, int bytes ) { 
  ...
  if( bytes <= 0 ) bytes = 1;
  if( !mem ) return(-1); 
  if( !*mem ) res = malloc( bytes );
  else res = realloc( *mem, bytes );
  if( res ) { *mem = res; return(0); }
  else mes_aux( MES_FLAG_TIME_WIN, 
...
int mes_fread_quiet( FILE* fp, void* mem, int bytes ) { 
  if( !bytes ) return(0); 
  if( mem && fp ) return( fread( mem, 1, bytes, fp ) );
  else mes_aux( MES_FLAG_TIME_WIN, 
...
int mes_fread( FILE* fp, void* mem, int bytes ) { 
  if( !bytes ) return(0); 
  if( mem && fp && fread( mem, 1, bytes, fp ) == (unsigned int)bytes ) return(0); 
...
int mes_fwrite( FILE* fp, void* mem, int bytes ) { 
  if( !fp || !mem  ) bytes = -1;
  else if( bytes < 0 ) bytes = strlen(mem);
  if( !bytes ) return(0);
  if( bytes > 0 && fwrite(mem, 1, bytes, fp) == (unsigned int)bytes ) return(0); 
  else mes_aux( MES_FLAG_TIME_WIN, 
...
int mes_fputc( FILE* fp, char chr ) { 
  if( fp && fputc( chr, fp ) != EOF ) return(0); 
...
int mes_fputs( FILE* fp, char* str ) { 
  if( fp && str && fputs( str, fp ) != EOF ) return(0); 
 
  if( str ) mes_aux(MES_FLAG_TIME_WIN, "fputs: could not write string %s\n", str);
  else      mes_aux(MES_FLAG_TIME_WIN, "fputs: could not write 0 pointer\n");
...
int mes_fgetc( FILE* fp ) { 
  ...
  else mes_aux( MES_FLAG_TIME_WIN, "fgetc: end of FILE(%p)\n", fp );
  return(res);  
} /* mes_fgetc */
...
int mes_fflush( FILE* fp ) { 
  ...
  if( !res ) return(0); 
  else mes_aux( MES_FLAG_TIME_WIN, "fflush: could not flush FILE(%p)\n", fp );
  return(res);  
} /* mes_fflush */
...
int mes_fprintf( FILE* fp, char* format, ... ) {
  ...
  if( fp && fp - stdout ) mes_fputs( fp, txt );
  else mes_win( txt );
  free( txt );
  return(1);  
} /* mes_fprintf */
...
int mes_fseek( FILE* fp, long offset, int fromwhere ) { 
  ...
  mes_aux( MES_FLAG_TIME_WIN, "fseek: could not position FILE(%p) at %ld", 
	   fp, offset );
  switch( fromwhere ) {
  case SEEK_SET : mes_aux( MES_FLAG_FILE_WIN, "\n" ); break; 
  case SEEK_END : mes_aux( MES_FLAG_FILE_WIN, " from the end\n" ); break; 
  case SEEK_CUR : mes_aux( MES_FLAG_FILE_WIN, " from current position\n" ); break;
  default : mes_aux(MES_FLAG_FILE_WIN, " with undefinded offset %d\n", fromwhere);
    break;
  }
  return(res);  
} /* mes_fseek */
...
STOP:
  ...
  mes_file_win( filename );
  mes_file_win( "\";");
  if( res != -1 ) mes_file_win( strerror(errno) );
  mes_file_win( "\n" );
  return(res);
#undef CUR_PROC
} /* mes_remove */
...
#define CUR_PROC "mes_rename"
  ...
  if( mes_filename_check(oldname) ) goto STOP; 
  if( mes_filename_check(newname) ) goto STOP; 
...
#define CUR_PROC "mes_move"
  ...
  if( mes_filename_check( oldname ) ) goto STOP;
  if( mes_filename_check( newname ) ) goto STOP;
  if( !strcmp( oldname, newname ) ) goto STOP;
  ...
  /* need to copy */
  if( mes_copy( oldname, newname ) ) goto STOP;
  mes_remove( oldname );
...
#define CUR_PROC "mes_copy"
  ...
  if( (dst = mes_fopen( newname, "wb" )) == NULL) goto STOP;
  if( (src = mes_fopen( oldname, "rb" )) == NULL) goto STOP;
  if( (buf = mes_malloc( 0x10000*sizeof(char) )) == NULL ) goto STOP;

  while (!feof(src)) {
    int cnt = fread( buf, 1, 0x10000, src );
    if( cnt<=0 ) break;
    if( fwrite( buf, 1, cnt, dst ) <= 0 ) goto STOP;
  }
  res = 0;
 STOP:
  if( buf ) free( buf );
...
# define CUR_PROC "mes_tmpfopen"
  ...
  if( path ) strncpy( tmpname, path, L_tmpnam );
  else       tmpname[0] = 0;
