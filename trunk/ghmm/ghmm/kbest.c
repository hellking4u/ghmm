...
#define CUR_PROC "kbest_buildLogMatrix"
  ...
  if (!m_malloc(log_a, N)) {mes_proc(); goto STOP;}
  for (i=0; i<N; i++) {
    if (!m_malloc(log_a[i], s[i].in_states)) {mes_proc(); goto STOP;}
...
#define CUR_PROC "kbest"
  ...
  if (!m_calloc(h, seq_len)) {mes_proc(); goto STOP;}
  ...
  hP = h[0];
  while (hP != NULL) {
    if (m_realloc(hP->gamma_a,  hP->gamma_states)) {mes_proc(); goto STOP;}
    if (m_realloc(hP->gamma_id, hP->gamma_states)) {mes_proc(); goto STOP;}
    hP = hP->next;
  }
  ...
  for (t=1; t<seq_len; t++) {
	...
	if (hP->gamma_a[i]>0.0) {mes_prot("gamma to large. kbest failed\n"); exit(1);}
      }
      hP = hP->next;
    }
    ...
    while (hP != NULL) {
      for (i=0; i<hP->gamma_states; i++) {
	i_id = hP->gamma_id[i];
	if (hP->gamma_a[i] > KBEST_EPS) continue;
	/* find first best hypothesis that is worse than current hypothesis: */
	for (l=0; l<k && maxima[i_id*k+l] < KBEST_EPS && maxima[i_id*k+l] > hP->gamma_a[i]; l++);
	if (l<k) {
    ...
    if (!h[t]) {mes_prot("No chosen hypothesis. kbest failed\n"); exit(1);}
    hP=h[t];
    while (hP->next != NULL) {
      if (1==hP->next->chosen)
      	hP = hP->next;
  ...
  *log_p = 1.0;   /* log_p will store log of maximum summed probability */
  ...
  if (*log_p < KBEST_EPS) {
    ...
    if (!m_malloc(hypothesis, seq_len)) {mes_proc(); goto STOP;}
