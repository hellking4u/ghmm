...
Main difference are :
 ...
 - Linux frendly icons (but removing lines add also a good perf bonus !).
...
Image are cached in a dict for speed up."""
  ...
  def __getitem__(self, name):
    try: return self.icons[name]
      ...
      file = os.path.join(self.path, name + ext)
...
"""
  ...
  def geticon(self):
    ...
    if not iconsdir:
      import os.path
      iconsdir = IconsDir(os.path.join(os.path.dirname(__file__), "icons"))
    if self.isexpandable():
