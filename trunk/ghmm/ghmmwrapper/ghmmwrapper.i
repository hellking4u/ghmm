...
// typedef struct model model; doppelt ?
...
/* Allocates memory for the silent array*/
/*extern int  model_initSilentStates(model *mo);*/
...
%inline %{
  ...
  /************ Create any sort of double[size] array ************/
  ...
  /************ Create any sort of long[size] array **************/
  ...
  /************ Create any sort of double[size1][size2] array ************/
  double **double_2d_array(int rows, int cols) {
    int i;
    double **arry = (double **) malloc(rows*sizeof(double*));
    for(i=0; i < rows; i++)
      arry[i] = double_array(cols);
    return arry;
  }
  ...
  /*
  int get_int_array_length(int *array){
	  int i,j,h;
	  i = sizeof(array);
	  j	= sizeof(array[0]);
	  h = sizeof(array)/sizeof(array[0]);
	  printf("%d / %d = %d", i,j,h);
	  return i;
  }	 */  
  ...
  /* initialization of state arrays */
  ...
  /************ create an array of pointers of pointers to sequence_d_t structs **************/
  ...
  sequence_d_t* get_seq_t_ptr(sequence_d_t** array, int index){
  ...
  extern int sequence_d_free(sequence_d_t **sq);
  ...
  /************ create an array of pointers to smosqd_t structs **************/
...
extern double model_prob_distance(model* m0, model* m, int maxT, int symmetric, int verbose);		   
		   

/*========================================================================*/

/*static int foba_initforward(model *mo, double *alpha_1, int symb, double
*scale);

static double foba_stepforward(state *s, double *alpha_t, const double
		b_symb);

int foba_forward(model *mo, const int *O, int len, double **alpha, 
		 double *scale, double *log_p);

int foba_descale(double **alpha, double *scale, int t, int n, double
		**newalpha); */
...
%inline %{
  ...
  // redundant, use get_smodel_ptr !!!
  //smodel* get_onesmodel(smodel** smos, int index) {
  //  return smos[index];
  //}
...
%inline %{
  ...
  void print_scluster(scluster_t *cl, sequence_d_t *seq,char *ch[]){
	  scluster_out(cl,seq, stdout, ch);
  }	  
